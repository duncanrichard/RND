...
<div class="w-full px-6 py-6 mx-auto mt-10">
    <div class="bg-white shadow-lg rounded-lg p-6">
        <h2 class="text-2xl font-bold mb-4">Tambah Data List Bahan Baku</h2>
        <form action="{{ route('master_bahan_baku.store') }}" method="POST" class="space-y-6">
            @csrf
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="jumlah_diterima" id="jumlah_diterima" 
                           class="mt-1 block w-full border border-gray-300 rounded shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" 
                           required>
                </div>
                <br>
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="4" 
                          class="mt-1 block w-full border border-gray-300 rounded shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
            </div>
            <br>
            <div>
...
<script>
    ...
    // Jika kode sudah ada dari backend, tampilkan langsung
    if (kodeInput && kodeInput.value === '') {
        fetch('/api/get-next-kode-bahan-baku') // Endpoint API untuk mendapatkan kode berikutnya
            .then(response => response.json())
            .then(data => {
                kodeInput.value = data.kode || 'S00001';
            })
            .catch(err => {
                console.error('Error fetching kode bahan baku:', err);
            });
    }
});
