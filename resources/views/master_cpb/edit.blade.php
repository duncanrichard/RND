...
<div class="w-full px-6 py-6 mx-auto mt-10">
    <div class="bg-white shadow-lg rounded-lg p-6">
        <h2 class="text-2xl font-bold mb-4">Tambah Data CPB</h2>
        <form action="{{ route('master_cpb.update', $item->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <!-- Form Pencarian -->
...
<div class="mb-4">
    ...
    <input type="text" name="kode_produk" id="kode_produk" value="{{ $item->kode_produk }}" class="w-full border p-2 rounded-lg" readonly required>
</div>

<div class="mb-4">
    <label for="nama_produk" class="block font-semibold">Nama Produk</label>
    <input type="text" name="nama_produk" id="nama_produk" value="{{ $item->nama_produk }}" class="w-full border p-2 rounded-lg" readonly required>
</div>

<div class="mb-4">
    <label for="batch_size_berat" class="block font-semibold">Batch Size Berat</label>
    <input type="number" name="batch_size_berat" id="batch_size_berat" value="{{ $item->batch_size_berat }}"class="w-full border p-2 rounded-lg" readonly required>
</div>

<div class="mb-4">
    <label for="batch_size_satuan" class="block font-semibold">Batch Size Satuan</label>
    <input type="text" name="batch_size_satuan" id="batch_size_satuan" value="{{ $item->batch_size_satuan }}" class="w-full border p-2 rounded-lg" readonly required>
</div>
<div class="mb-4">
                <label for="nomor_cpb" class="block font-semibold">Nomor CPB</label>
                <input type="text" name="nomor_cpb" id="nomor_cpb" class="w-full border p-2 rounded-lg" value="{{ $item->nomor_cpb }}" required>
            </div>
<div class="mb-4">
                <label for="file_dokumen" class="block font-semibold">Upload Dokumen Baru (PDF)</label>
                <input type="file" name="file_dokumen" id="file_dokumen" class="w-full border p-2 rounded-lg" accept=".pdf">
                @if ($item->file_dokumen)
                <p class="mt-2 text-sm text-gray-600">Dokumen saat ini: <a href="{{ asset('storage/' . $item->file_dokumen) }}" target="_blank" class="text-blue-500 underline">Lihat Dokumen</a></p>
                @endif
            </div>
...
<input type="hidden" name="id_kode_produk" id="id_kode_produk">
            ...
            <div class="mt-6">
                ...
                <a href="{{ route('master_cpb.index') }}" class="w-full font-bold px-4 py-2 text-white rounded-lg bg-gray-500 hover:bg-gray-600 mt-2 inline-block text-center" style="background-color: #032859;">Batal</a>
            </div>
        </form>
    </div>
</div>
