...
<div class="w-full px-6 py-6 mx-auto mt-10">
    <div class="bg-white shadow-lg rounded-lg p-6">
        <h2 class="text-2xl font-bold mb-4">Update Data Harga Sample Bahan Baku</h2>
 <!-- Form Search -->
 <div class="mb-6">
       ...
       <div class="overflow-x-auto mb-6">
        ...
        <!-- Form Update Data -->
        <form action="{{ route('master_harga_sample_bahan_baku.update', $data->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-2 gap-4">
            <div>
    <label for="kode_bahan_baku" class="block text-sm font-medium text-gray-700">Kode Bahan Baku</label>
    <input type="text" name="kode_bahan_baku" id="kode_bahan_baku" 
        class="mt-1 block w-full border border-gray-300 rounded shadow-sm py-2 px-3" 
        value="{{ $data->kode_bahan_baku }}" readonly required>
</div>

                <br>
            <div>
                    <label for="nama_bahan_baku" class="block text-sm font-medium text-gray-700">Nama Bahan Baku</label>
                    <input type="text" name="nama_bahan_baku" id="nama_bahan_baku" class="mt-1 block w-full border border-gray-300 rounded shadow-sm py-2 px-3" value="{{ $data->nama_bahan_baku }}" readonly required>
                </div>
                <br>
                <div>
                    <label for="principle" class="block text-sm font-medium text-gray-700">Principle</label>
                    <input type="text" name="principle" id="principle" class="mt-1 block w-full border border-gray-300 rounded shadow-sm py-2 px-3" value="{{ $data->principle }}" readonly required>
                </div>
                <br>
                <div>
                    <label for="supplier" class="block text-sm font-medium text-gray-700">Supplier</label>
                    <input type="text" name="supplier" id="supplier" class="mt-1 block w-full border border-gray-300 rounded shadow-sm py-2 px-3" value="{{ $data->supplier }}" readonly required>
                </div>

                <br>
                <div>
                    <label for="qty" class="block text-sm font-medium text-gray-700">Qty</label>
                    <input type="number" step="0.01" name="qty" id="qty" class="mt-1 block w-full border border-gray-300 rounded shadow-sm py-2 px-3" value="{{ $data->qty }}" required>
                </div>
                <br>
                <div>
                    ...
                    <select name="kategori_satuan" id="kategori_satuan" class="mt-1 block w-full border border-gray-300 rounded shadow-sm py-2 px-3" required>
                        ...
                        @foreach($satuan as $item)
                            <option value="{{ $item->id }}" {{ $data->kategori_satuan == $item->id ? 'selected' : '' }}>
                                {{ $item->nama_satuan }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="moq" id="moq" class="mt-1 block w-full border border-gray-300 rounded shadow-sm py-2 px-3" value="{{ $data->moq }}">
                </div>
            </div>

            <br>
            <input type="hidden" name="harga_idr" id="harga_idr" value="{{ $data->harga_idr ?? 0 }}">
            <input type="hidden" name="ppn" id="ppn" value="{{ $data->ppn ?? 0 }}">
            <input type="hidden" name="pph" id="pph" value="{{ $data->pph ?? 0 }}">
            <input type="hidden" name="harga_total" id="harga_total" value="{{ $data->harga_total ?? 0 }}">
            <input type="hidden" name="harga_akhir" id="harga_akhir" value="{{ $data->harga_akhir ?? 0 }}">
...
<script>
  document.getElementById('search_button').addEventListener('click', function() {
    ...
    // Inisialisasi Select2 untuk Kategori Satuan
    $('#kategori_satuan').select2({
        placeholder: "Pilih Kategori Satuan",
        minimumInputLength: 1,
        allowClear: true,
        ajax: {
            url: "{{ route('get-kategori-satuan') }}",
            dataType: 'json',
            delay: 250,
...
const latestKursUSD = {{ $latestKursUSD ?? 0 }};
    ...
    const latestAdditionalCostRate = {{ $latestAdditionalCost ?? 0 }} / 100;

    function formatRupiah(value) {
        return new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(value);
    }

    // Perhitungan Harga IDR dari USD
    ...
    // Perhitungan Pajak dan Harga Total
    ...
    // Perhitungan Harga Akhir dengan Additional Cost
    function calculateHargaAkhir(hargaTotal) {
        const additionalCost = hargaTotal * latestAdditionalCostRate;
    ...
    // Event Listener untuk Harga IDR
